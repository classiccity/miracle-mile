...
final class FLThemeUpdate {
	...
	static public function init() {
		...
		// Update to 1.7.20
		if ( version_compare( $saved_version, '1.7.20', '<' ) ) {
			self::v_1_7_20();
		}
		...
		/**
		 * Fires after the theme has been updated to the current version.
		 */
	...
	/**
	 * Updates to version 1.7.20. Bootstrap 4 Minimal becomes the new default
	 * framework. For existing sites that never explicitly set the framework,
	 * lock them to Bootstrap 3 Minimal to avoid a silent switch.
	 *
	 * @since 1.7.20
	 * @access private
	 * @return void
	 */
	static private function v_1_7_20() {
		$saved_version = self::get_saved_version();

		if ( $saved_version ) {
			$mods = get_theme_mods();
			if ( ! isset( $mods['fl-framework'] ) ) {
				set_theme_mod( 'fl-framework', 'base' );
			}
		}
	}
