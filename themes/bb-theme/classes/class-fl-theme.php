...
final class FLTheme {
	...
	static public function enqueue_scripts() {
		...
		/**
		 * Whether to disable smooth scrolling for anchor links.
		 */
		...
		// Lightbox
		$themeopts['lightbox'] = self::get_setting( 'fl-lightbox' );

		// Scroll position for when to display the Scroll To Top button.
		$themeopts['scrollTopPosition'] = '800';
		/**
		 * The theme options array passed to the front-end JavaScript.
		 */
		$themeopts = apply_filters( 'fl_theme_themeopts', $themeopts );
		/**
		 * The CSS/JS framework to enqueue (e.g. 'bootstrap' or 'base').
		 */
		...
		if ( 'file' === FLTheme::get_asset_enqueue_method() ) {
			wp_enqueue_style( 'fl-automator-skin', FLCustomizer::css_url(), array( $framework ), FL_THEME_VERSION );
		} else {
			wp_register_style( 'fl-automator-skin', false, array( $framework ) );
			wp_enqueue_style( 'fl-automator-skin' );
			wp_add_inline_style( 'fl-automator-skin', self::get_cached_css( true === FLCustomizer::is_customizer_preview() ? 'customizer' : 'skin' ) );
		}
	...
	static public function get_theme_breakpoints() {
		...
		/**
		 * The responsive breakpoint values passed to the front-end JavaScript.
		 */
	...
	static public function title() {
		if ( ! function_exists( '_wp_render_title_tag' ) ) {
			...
			/**
			 * The separator string used between the page title and site name in the <title> tag.
			 */
			...
			/**
			 * The full page title string rendered in the <title> tag.
			 */
	...
	static public function fonts() {
		...
		/**
		 * The font weight variants to load for the body font family.
		 */
	...
	static public function add_font( $name, $variants = array() ) {
		/**
		 * The base domain URL used to load Google Fonts stylesheets.
		 */
	...
	static public function render_fonts() {
		foreach ( self::$fonts as $name => $font ) {
			if ( ! empty( $font['url'] ) ) {
				/**
			 * The Unicode subset string to append to the Google Fonts URL for the given font.
			 */
	...
	static public function head() {
		...
		if ( ! empty( $settings['fl-head-code'] ) ) {
			echo do_shortcode( $settings['fl-head-code'] ) . "\n";
		}

		/**
		 * Fires inside the document <head> element.
		 */
		do_action( 'fl_head' );
	}
	...
	static public function body_class( $classes ) {
		...
		/**
		 * Whether the theme header is enabled.
		 */
	...
	static public function header_code() {
		echo do_shortcode( self::get_setting( 'fl-header-code' ) );
	}
	...
	static public function fixed_header() {
		...
		/**
		 * Whether the fixed (sticky) header is enabled.
		 */
	...
	static public function fixed_header_logo() {
		...
		if ( $sticky_logo && 'fadein' === $header_fixed && 'image' === $logo_type ) {
			...
			/**
			 * The title attribute for the logo image.
			 */
	...
	static public function header_content() {
		...
		/**
		 * Fires at the opening of the header content section.
		 */
		...
		/**
		 * Fires at the closing of the header content section.
		 */
	...
	static public function logo() {
		...
		if ( function_exists( 'apply_filters_deprecated' ) ) {
			$logo_text = apply_filters_deprecated( 'fl-logo-text', array( self::get_setting( 'fl-logo-text' ) ), '1.6.3', 'fl_logo_text' );
				...
				/**
			 * The text logo string (deprecated; use fl_logo_text instead).
			 */
		...
		if ( 'image' === $logo_type ) {
			...
			$image_data = self::get_logo_dimensions( $logo_image );
			/**
			 * The alt text for the logo image.
			 */
			$logo_text = apply_filters( 'fl_logo_text', get_bloginfo( 'name' ) );
			...
			/**
			 * The CSS class applied to the logo image element.
			 */
			$logo_image_class = apply_filters( 'fl_logo_image_class', 'fl-logo-img' );
			/**
			 * The HTML attributes array applied to the logo image element.
			 */
			...
			if ( $mobile_logo ) {
				$mobile_data = self::get_logo_dimensions( $mobile_logo );
				$width       = isset( $mobile_data['width'] ) ? $mobile_data['width'] : '';
				$height      = isset( $mobile_data['height'] ) ? $mobile_data['height'] : '';
				if ( $width && $height ) {
					printf( ' data-mobile="%s" data-mobile-width="%s" data-mobile-height="%s"', $mobile_logo, $width, $height );
				} else {
					printf( ' data-mobile="%s"', $mobile_logo );
				}
			}
			echo ' title="' . esc_attr( $logo_title ) . '"';
			echo ( isset( $image_data['width'] ) && ! empty( $image_data['width'] ) ) ? sprintf( ' width="%1$s" data-width="%1$s"', $image_data['width'] ) : '';
			echo ( isset( $image_data['height'] ) && ! empty( $image_data['width'] ) ) ? sprintf( ' height="%1$s" data-height="%1$s"', $image_data['height'] ) : '';
			echo ' alt="' . esc_attr( $logo_text ) . '" />';
			echo '<meta itemprop="name" content="' . esc_attr( $logo_text ) . '" />';
		} else {
	...
	/**
	 * Returns cached width/height for a logo image URL.
	 *
	 * Caches the resolved attachment dimensions so we don't run
	 * attachment_url_to_postid() (an unbounded postmeta scan in WP core)
	 * on every front-end render. See issue #483.
	 *
	 * @since 1.7.20
	 * @param string $url Logo image URL.
	 * @return array {
	 *     @type string|int $width
	 *     @type string|int $height
	 * }
	 */
	static private function get_logo_dimensions( $url ) {
		if ( empty( $url ) ) {
			return array(
				'width'  => '',
				'height' => '',
		...
		// A failed lookup (e.g. during an HTTP->HTTPS or CDN URL migration where
		// attachment_url_to_postid() can't match a valid attachment) returns
		// empty dimensions. Cache that only briefly so the <img> recovers within
		// an hour once the URL resolves again, instead of being stuck for a week.
		$ttl = ( '' !== $dims['width'] || '' !== $dims['height'] ) ? WEEK_IN_SECONDS : HOUR_IN_SECONDS;
		set_transient( $key, $dims, $ttl );
		return $dims;
	}

	/**
	 * Clears cached logo dimension transients when Customizer settings save.
	 *
	 * Hooked to customize_save, which fires before the new theme mods are
	 * committed, so get_setting() still returns the outgoing logo URLs. That is
	 * what we need to clear: the cache keyed on the URL that was rendering. If
	 * the logo URL is unchanged the same key is cleared and re-primed on the
	 * next render, which also picks up a replaced image at the same URL.
	 *
	 * @since 1.7.20
	 * @return void
	 */
	static public function clear_logo_dimension_cache() {
		$urls = array(
			self::get_setting( 'fl-logo-image' ),
			self::get_setting( 'fl-mobile-header-logo' ),
		);
		foreach ( $urls as $url ) {
			if ( ! empty( $url ) ) {
				delete_transient( 'fl_logo_dim_' . md5( $url ) );
			}
		}
	}
	...
	static public function nav_menu_fallback( $args ) {
		...
		/**
		 * The URL used in the nav menu fallback link when no menu is assigned.
		 */
	...
	static public function nav_toggle_text() {
		...
		if ( 'icon' === $type ) {
			FLTheme::enqueue_fontawesome();
			/**
			 * The HTML for the mobile navigation toggle icon.
			 */
		...
		/**
		 * The HTML output for the mobile navigation toggle button.
		 */
	...
	static public function social_icons( $circle = true ) {
		...
		/**
		 * The list of social icon keys to display in the header or footer.
		 */
	...
	static public function has_footer() {
		...
		/**
		 * Whether the theme footer is enabled.
		 */
	...
	static public function footer_code() {
		echo do_shortcode( self::get_setting( 'fl-footer-code' ) );
	}
	...
	static public function post_navigation() {
		...
		if ( 'visible' === $show_nav ) {
			/**
			 * Whether post navigation links should be restricted to the same taxonomy term.
			 */
	...
	static public function go_to_top() {
		if ( self::get_setting( 'fl-scroll-to-top' ) === 'enable' ) {
			FLTheme::enqueue_fontawesome();
			printf( '<a href="#" role="button" id="fl-to-top"><span class="sr-only">%s</span><i class="fas fa-chevron-up" aria-hidden="true"></i></a>', esc_attr__( 'Scroll To Top', 'fl-automator' ) );
		}
	}
	...
	/**
	 * Builds the comment form fields for the comment form.
	 *
	 * @since 1.7.20
	 * @param array $arguments The arguments for the comment form field.
	 * @return string The HTML for the comment form field.
	 */
	static public function comment_form_field_helper( $arguments ) {
		$asterisk   = '';
		$attributes = array( 'class="form-control"' );
		foreach ( $arguments as $key => $value ) {
			if ( in_array( $key, array( 'type', 'label', 'required' ), true ) ) {
				continue;
			}
			$attributes[] = sprintf( '%s="%s"', $key, esc_attr( $value ) );
		}
		if ( $arguments['required'] ) {
			$asterisk     = '<abbr aria-hidden="true">*</abbr>';
			$attributes[] = 'required';
		}
		$label = sprintf( '<label for="%s">%s%s</label>', $arguments['id'], $arguments['label'], $asterisk );
		if ( 'textarea' === $arguments['type'] ) {
			return $label . sprintf( '<textarea %s></textarea><br />', implode( ' ', $attributes ) );
		}
		return $label . sprintf( '<input type="%s" %s /><br />', $arguments['type'], implode( ' ', $attributes ) );
	}
	...
	static public function comment_form_default_fields( $fields ) {
		...
		$fields['author'] = self::comment_form_field_helper( array(
			'id'           => 'fl-author',
			'type'         => 'text',
			'name'         => 'author',
			'label'        => _x( 'Name', 'Comment form label: comment author name.', 'fl-automator' ),
			'value'        => $commenter['comment_author'],
			'required'     => $required,
			'autocomplete' => 'name',
		) );
		$fields['email']  = self::comment_form_field_helper( array(
			'id'           => 'fl-email',
			'type'         => 'email',
			'name'         => 'email',
			'label'        => _x( 'Email (will not be published)', 'Comment form label: comment author email.', 'fl-automator' ),
			'value'        => $commenter['comment_author_email'],
			'required'     => $required,
			'autocomplete' => 'email',
		) );
		$fields['url']    = self::comment_form_field_helper( array(
			'id'           => 'fl-url',
			'type'         => 'text',
			'name'         => 'url',
			'label'        => _x( 'Website', 'Comment form label: comment author website.', 'fl-automator' ),
	...
	static public function print_schema( $schema, $echo = true ) {
		if ( self::is_schema_enabled() ) {
			/**
			 * The schema markup string before it is echoed or returned.
			 */
	...
	public static function skip_to_link() {
		$fixed_header = self::get_setting( 'fl-fixed-header' );
	...
	public static function comment_form_defaults( $args ) {
		...
		$texts   = array(
			...
			'login'  => _x( 'You must be <a%s>logged in</a> to post a comment.', 'Please, keep the HTML tags.', 'fl-automator' ),
			/* translators: %s: the logged in username */
			'logged' => __( 'Logged in as %s.', 'fl-automator' ),
			'logout' => __( 'Log out &raquo;', 'fl-automator' ),
		);
		$links   = array(
			'profile' => esc_url( admin_url( 'profile.php' ) ),
			'login'   => esc_url( wp_login_url( get_permalink() ) ),
			'logout'  => esc_url( wp_logout_url( get_permalink() ) ),
		);
		$comment = array(
			'id'       => 'fl-comment',
			'type'     => 'textarea',
			'name'     => 'comment',
			'label'    => _x( 'Comment', 'Comment form label: comment content.', 'fl-automator' ),
			'required' => true,
			'cols'     => 60,
			'rows'     => 8,
		);
		$profile = sprintf( '<a href="%s">%s</a>', $links['profile'], esc_html( $user_identity ) );
		$login   = sprintf( $texts['login'], ' href="' . $links['login'] . '"' );
		$logged  = sprintf( $texts['logged'], $profile );
		$logout  = sprintf( '<a href="%s">%s</a>', $links['logout'], $texts['logout'] );
		$args    = array(
			...
			'comment_notes_after'  => '',
			'comment_field'        => self::comment_form_field_helper( $comment ),
			'must_log_in'          => sprintf( '<p>%s</p>', $login ),
			'logged_in_as'         => sprintf( '<p>%s %s</p>', $logged, $logout ),
		);
		return $args;
	}

	/**
	 * Get top-level menu items from HTML string.
	 *
	 * @since 1.0.0
	 * @param string $html The HTML string containing menu items.
	 * @return array Array of top-level li elements.
	 */
	static public function get_top_level_menu_items( $html ) {
		...
		while ( $position < strlen( $clean_html ) ) {
			$li_start = strpos( $clean_html, '<li', $position );
			if ( false === $li_start ) {
				break;
			}

			// Determine if this li element is at the root level
			$before_li = substr( $clean_html, 0, $li_start );
			$open_li   = substr_count( $before_li, '<li' );
			$close_li  = substr_count( $before_li, '</li>' );

			if ( $open_li === $close_li ) {
				// Extract the complete li element including any nested content
				$remaining = substr( $clean_html, $li_start );
				$depth     = 0;
				$li_end    = $li_start;

				// Parse through content to find matching closing tag
				for ( $i = 0; $i < strlen( $remaining ); $i++ ) {
					if ( substr( $remaining, $i, 3 ) === '<li' ) {
						$depth++;
					} elseif ( substr( $remaining, $i, 5 ) === '</li>' ) {
						$depth--;
						if ( 0 === $depth ) {
							$li_end = $li_start + $i + 5;
							break;
				...
				if ( $li_end > $li_start ) {
					$full_li           = substr( $clean_html, $li_start, $li_end - $li_start );
					$top_level_items[] = $full_li;
					$position          = $li_end;
				} else {
					$position++;
				}
	...
	/**
	 * Add inline logo to centered menu layout.
	 *
	 * @since 1.0.0
	 ...
	 */
	static public function add_inline_logo_menu_item( $items, $args ) {
		if ( 'centered-inline-logo' === get_theme_mod( 'fl-header-layout', '' ) &&
			'header' === $args->theme_location &&
			false === strpos( $args->items_wrap, 'navbar-right' ) &&
			! wp_is_mobile() && empty( $args->walker ) ) {

			$menu_items = self::get_top_level_menu_items( $items );
			...
			$custom_item = '<li class="fl-logo-centered-inline"><div class="fl-page-header-logo fl-inline-logo-' . FLTheme::get_setting( 'fl-inline-logo-side' ) . ' col-sm-12"' . FLTheme::print_schema( ' itemscope="itemscope" itemtype="https://schema.org/Organization"', false ) . '>
				<a href="' . esc_url( home_url( '/' ) ) . '" itemprop="url">' . $logo . '</a>
			</div></li>';

			$count     = count( $menu_items );
			$logo_side = get_theme_mod( 'fl-inline-logo-side', 'right' );

			// Calculate insertion position for logo placement
			if ( 'left' === $logo_side ) {
				$insert_position = floor( $count / 2 );
			} elseif ( 'right' === $logo_side ) {
				if ( 0 !== $count % 2 ) {
					$insert_position = floor( $count / 2 ) + 1;
				} else {
					$insert_position = floor( $count / 2 );
				}
			}

			array_splice( $menu_items, $insert_position, 0, array( $custom_item ) );
	...
	/**
	 * Declare colours used in theme json filter
	 * @since 1.7
	 */
	public static function wp_theme_json_data_theme() {
		$mods = FLCustomizer::get_mods();

		$settings = array(
			'fl-heading-text-color'         => __( 'Heading Text', 'fl-automator' ),
			'fl-body-bg-color'              => __( 'Body Background', 'fl-automator' ),
			'fl-body-text-color'            => __( 'Body Text', 'fl-automator' ),
			'fl-accent'                     => __( 'Accent', 'fl-automator' ),
			'fl-accent-hover'               => __( 'Accent Hover', 'fl-automator' ),
			'fl-topbar-bg-color'            => __( 'Topbar Background', 'fl-automator' ),
			'fl-topbar-text-color'          => __( 'Topbar Text', 'fl-automator' ),
			'fl-topbar-link-color'          => __( 'Topbar Link', 'fl-automator' ),
			'fl-topbar-hover-color'         => __( 'Topbar Hover', 'fl-automator' ),
			'fl-header-bg-color'            => __( 'Header Background', 'fl-automator' ),
			'fl-header-text-color'          => __( 'Header Text', 'fl-automator' ),
			'fl-header-link-color'          => __( 'Header Link', 'fl-automator' ),
			'fl-header-hover-color'         => __( 'Header Hover', 'fl-automator' ),
			'fl-nav-bg-color'               => __( 'Nav Background', 'fl-automator' ),
			'fl-nav-link-color'             => __( 'Nav Link', 'fl-automator' ),
			'fl-nav-hover-color'            => __( 'Nav Hover', 'fl-automator' ),
			'fl-content-bg-color'           => __( 'Content Background', 'fl-automator' ),
			'fl-footer-widgets-bg-color'    => __( 'Footer Widgets Background', 'fl-automator' ),
			'fl-footer-widgets-text-color'  => __( 'Footer Widgets Text', 'fl-automator' ),
			'fl-footer-widgets-link-color'  => __( 'Footer Widgets Link', 'fl-automator' ),
			'fl-footer-widgets-hover-color' => __( 'Footer Widgets Hover', 'fl-automator' ),
			'fl-footer-bg-color'            => __( 'Footer Background', 'fl-automator' ),
			'fl-footer-text-color'          => __( 'Footer Text', 'fl-automator' ),
			'fl-footer-link-color'          => __( 'Footer Link', 'fl-automator' ),
			'fl-footer-hover-color'         => __( 'Footer Hover', 'fl-automator' ),
		);

		$pallete = array();

		foreach ( $settings as $setting => $name ) {
			if ( isset( $mods[ $setting ] ) && ! empty( $mods[ $setting ] ) ) {
				$pallete[] = array(
					'slug'  => str_replace( '-color', '', $setting ),
					'name'  => $name,
					'color' => $mods[ $setting ],
				);
			}
		}
		add_theme_support( 'editor-color-palette', $pallete );
	}

	public static function global_less( $less ) {
		if ( class_exists( 'FLBuilderGlobalStyles' ) ) {
			$settings = FLBuilderGlobalStyles::get_settings( false );
			$colors   = (array) $settings->colors;
			foreach ( $colors as $color ) {
				if ( empty( $color ) || strstr( $color['color'], 'hsl' ) ) {
					continue;
				}
				$slug  = FLBuilderGlobalStyles::label_to_key( $color['label'] );
				$color = FLBuilderColor::hex_or_rgb( $color['color'] );
				$less .= "body.has-blocks .has-$slug-color{color: $color;}body.has-blocks .has-$slug-background-color {background-color: $color;}";
			}
		}
		return $less;
	}
